...
export const AdminAnnouncementsScreen: React.FC = () => {
 ...
 <p className="text-xs text-subtle">Publish broadcast banners displayed above student feeds</p>
 </div>
 ...
 <Megaphone className="w-8 h-8 text-subtle mx-auto mb-2 opacity-50" />
 ...
 <Badge variant={item.type === 'warning' ? 'danger' : item.type === 'success' ? 'success' : 'default'}>
 {item.type.toUpperCase()}
 </Badge>
 {item.isActive ? <Badge variant="success">ACTIVE</Badge> : <Badge variant="default">INACTIVE</Badge>}
 <span className="text-[11px] text-subtle">{timeAgo(item.createdAt)}</span>
 </div>
