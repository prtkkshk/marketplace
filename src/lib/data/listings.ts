...
export async function fetchListings(params: FetchListingsParams = {}): Promise<{ listings: ListingItem[]; hasMore: boolean }> {
  ...
  let query = supabase
    .from('listings')
    .select(
      'id, user_id, title, description, category, price, is_negotiable, condition, photo_paths, hall_of_residence, status, is_pinned, sold_at, deleted_at, expires_at, created_at, updated_at, profiles!inner(full_name, hall_of_residence)'
    )
    .is('deleted_at', null);
