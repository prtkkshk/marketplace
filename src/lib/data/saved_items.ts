...
export async function fetchSavedListings(userId: string): Promise<ListingItem[]> {
  const { data, error } = await supabase
    .from('saved_items')
    .select(
      'listing_id, listings!inner(id, user_id, title, description, category, price, is_negotiable, condition, photo_paths, hall_of_residence, status, is_pinned, sold_at, deleted_at, expires_at, created_at, updated_at, profiles!user_id(full_name, hall_of_residence))'
    )
    .eq('user_id', userId)
